...
class LinearizedMutilSampler():
    ...
    def linearized_fitting(cls, input, grid):
        ...
        if cls.fixed_bias:
            center_grid_xyz_stop = torch.cat(
                [center_grid_stop, torch.ones(center_grid_stop[..., 0:1].shape, device=center_grid_stop.device)],
                dim=4).permute(0, 2, 3, 4, 1)
        else:
            center_grid_xyz_stop = torch.cat(
                [center_grid_stop, torch.zeros(center_grid_stop[..., 0:1].shape, device=center_grid_stop.device)],
                dim=4).permute(0, 2, 3, 4, 1)

        # map to linearized, equation(2) in paper
        image_linearized = \
        torch.matmul(gradient_intensity_stop.transpose(3, 4), (center_grid_xyz - center_grid_xyz_stop))[..., 0].permute(
            0, 3, 1, 2) + center_image[:, 0]
    ...
    def get_delta_vals(data_dict):
        ...
        delta_mask = (delta_grid[..., 0:1] >= -1.0) * (delta_grid[..., 0:1] <= 1.0) * (delta_grid[..., 1:2] >= -1.0) * (
                    delta_grid[..., 1:2] <= 1.0)
    ...
    def add_noise_to_grid(cls, grid, least_offset):
        ...
        grid_noise = torch.randn([batch_size, cls.num_grid - 1, height, width, num_xy],
                                 device=grid.device) / torch.tensor([[width, height]], dtype=torch.float32,
