...
import utils
from bohb_utils import pretrain, evaluate


def objective_function(config, budget, args):
    print("============================================")
    print("            NEXT BOHB ITERATION             ")
    print("============================================")
    args.epochs = budget
    if config:
        for k, v in config:
    ...
    un_iq = hash(config)
    args.exp_dir = Path(args.output_dir) / str(un_iq)
    ...
    ckp_path = os.path.join(args.exp_dir, "checkpoint.pth.tar")
    ckp = torch.load(ckp_path, map_location="cpu")
    score = ckp["best_acc"] / 100

    print("============================================")
    print("            BOHB ITERATION END              ")
    print("============================================")
...
def get_configspace(seed=None):
    ...
    lr = csh.UniformFloatHyperparameter("lr", lower=1e-6, upper=0.1, log=True, default_value=1e-3)
    cs.add_hyperparameters([lr])

    penalty = csh.CategoricalHyperparameter("stn_penalty", choices=["OverlapPenalty", "ThetaLoss", "ThetaCropsPenalty"])
    cs.add_hyperparameter(penalty)

    eps = csh.UniformFloatHyperparameter("epsilon", lower=1, upper=1000, default_value=1)
    cs.add_hyperparameters([eps])

    stn_optimizer = csh.CategoricalHyperparameter('use_stn_optimizer', choices=[True, False], default_value=False)
    cs.add_hyperparameter(stn_optimizer)

    stn_lr = csh.UniformFloatHyperparameter('stn_lr', lower=1e-6, upper=1., log=True, default_value=0.0001)
    cs.add_hyperparameter(stn_lr)

    cond = InCondition(stn_lr, stn_optimizer, [True])
    cs.add_condition(cond)

    stn_mode = csh.CategoricalHyperparameter(
        'stn_mode',
        choices=["affine",
                 "rotation_translation", "rotation_translation_scale", "rotation_translation_scale_symmetric",
                 "translation", "translation_scale", "translation_scale_symmetric"],
        default_value="translation_scale_symmetric")
    cs.add_hyperparameter(stn_mode)

    return cs


def main(args):
    cs = get_configspace(args)

    scenario = Scenario({
        "run_obj": "quality",  # we optimize quality (alternative to runtime)
        "n_trials": 1000,
        "cs": cs,  # configuration space
        "output-dir": args.output_dir,  # working directory where intermediate results are stored
        "seed": args.seed,
    })

    intensifier_kwargs = {'initial_budget': 10, 'max_budget': 100, 'eta': 3}
    smac = SMAC4MF(
        scenario=scenario,
        rng=np.random.RandomState(args.seed),
        tae_runner=objective_function,
        tae_runner_kwargs={"args": args},
...
def get_args_parser():
    ...
    parser.add_argument("--stn_penalty", default=None, type=str,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('DINO-STN-BOHB', parents=[get_args_parser()])
